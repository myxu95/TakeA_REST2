...
class StructureAnalyzer:
    ...
    def _load_universe(self) -> mda.Universe:
        ...
        try:
            # Check if we need to extract PDB with chain information
            if self.topology_file.suffix == '.tpr':
                # Try to create PDB file with chain information from TPR
    ...
    def _extract_pdb_with_chains(self) -> None:
        """Extract PDB file with chain information from TPR"""
        try:
            import subprocess
            
            pdb_file = self.structure_file.with_suffix('.pdb')
            
            # Run gmx trjconv to extract PDB with chain information
            cmd = [
                'gmx', 'trjconv',
                '-f', str(self.structure_file),
                '-s', str(self.topology_file),
                '-o', str(pdb_file),
                '-pbc', 'mol'
            ]
            
            print(f"Extracting PDB with chain information from TPR file...")
            ...
            if result.returncode == 0 and pdb_file.exists():
                print(f"✓ Extracted PDB with chain information: {pdb_file}")
            else:
                print(f"Warning: Could not extract PDB with chains")
            
        except Exception as e:
            print(f"Warning: Could not extract PDB with chains: {e}")
            return
