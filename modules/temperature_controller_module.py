...
class TemperatureController:
    ...
    def __init__(self, config_manager, replica_data: Dict[str, Any], 
        ...
        if solute_data and 'solute_atom_indices' in solute_data:
            ...
            self.solute_atom_indices = [idx + 1 for idx in solute_data['solute_atom_indices']]
    ...
    def _create_replica_topology(self, base_topology: Path, output_topology: Path,
        ...
        header = f"""; REST2 Topology for Replica {replica_index}
; Temperature: {self.temperatures[replica_index]:.1f} K
; Scaling factor (λ): {self.scaling_factors[replica_index]:.6f}
; 
; NOTE: REST2 scaling is handled by PLUMED PARTIAL_TEMPERING
; The marked atoms (with '_' suffix) will be scaled by PLUMED
; Generated automatically

"""
        
        # Write topology with header
...
"""
    ...
    def prepare_additional_input_files(self) -> None:
        """Prepare additional input files needed for each replica"""
        for replica in self.replicas:
            replica_index = replica['index']
            input_dir = Path(replica['input_dir'])
            
            # Create index file if needed
            self._create_replica_index_file(input_dir, replica_index)
            
            # Copy and customize PLUMED file if exists
            self._prepare_plumed_file(input_dir, replica_index)
    
    def _create_replica_index_file(self, input_dir: Path, replica_index: int) -> None:
        """
        Create index file for replica if needed
        
        Args:
            input_dir: Replica input directory
            replica_index: Replica index
        """
        # Check if source index file exists
        source_index = None
        md_results_dir = self.config.get_parameter('md_results_dir')
        if md_results_dir:
            potential_index = Path(md_results_dir) / "index.ndx"
            if potential_index.exists():
                source_index = potential_index
        
        if source_index:
            # Copy existing index file
            shutil.copy2(source_index, input_dir / "index.ndx")
        else:
            # Create basic index file
            index_content = f"""; Index file for REST2 replica {replica_index}
; Generated automatically

[ System ]
; All atoms in the system - will be auto-generated by GROMACS if needed

[ Protein ]
; Protein atoms - will be auto-generated by GROMACS if needed

[ Non-Protein ]
; Non-protein atoms - will be auto-generated by GROMACS if needed
"""
            with open(input_dir / "index.ndx", 'w') as f:
    ...
    def _prepare_plumed_file(self, input_dir: Path, replica_index: int) -> None:
        ...
        # Create PLUMED file even if original doesn't exist
        temperature = self.temperatures[replica_index]
        scaling_factor = self.scaling_factors[replica_index]
        
        # Add replica-specific header
        header = f"""# PLUMED input file for REST2 replica {replica_index}
# Temperature: {temperature:.1f} K
# Scaling factor (λ): {scaling_factor:.6f}
# Generated automatically
...
"""
        ...
        # Read and customize original PLUMED content if exists
        original_content = ""
        if plumed_dat and Path(plumed_dat).exists():
            with open(plumed_dat, 'r') as f:
                original_content = f.read()
            # Customize output file names
            original_content = self._customize_plumed_outputs(original_content, replica_index)
        
        # Write customized PLUMED file
    ...
    def _create_partial_tempering_command(self, replica_index: int) -> str:
        ...
        if self.solute_atom_indices and len(self.solute_atom_indices) > 0:
            # Use actual solute atoms
            atom_list = self._format_atom_list(self.solute_atom_indices)
        else:
            # Fallback: use a placeholder that needs to be manually edited
            atom_list = "1-100  # EDIT THIS: Replace with actual solute atom indices"
        
        partial_tempering_cmd = f"""# REST2 implementation using PARTIAL_TEMPERING
PARTIAL_TEMPERING ...
  ATOMS={atom_list}
  TEMP={self.T_ref:.1f}
...
"""
    ...
    def _format_atom_list(self, atom_indices: List[int]) -> str:
        ...
        # Join ranges, with line breaks for readability if too long
        atom_list = ",".join(ranges)
        if len(atom_list) > 60:  # Break long lines
            formatted_ranges = []
            current_line = ""
            for range_str in ranges:
                if len(current_line + "," + range_str) > 60:
...
"""
    ...
    def validate_temperature_setup(self) -> bool:
        """
        Validate that temperature setup is complete
        ...
        for replica in self.replicas:
            ...
            # Validate topology file contains temperature information
            topology_file = input_dir / "topol.top"
            if topology_file.exists():
                with open(topology_file, 'r') as f:
                    content = f.read()
                    if f"Replica {replica_index}" not in content:
                        errors.append(f"Replica {replica_index}: Topology file missing replica-specific header")
            
            # Validate MDP file contains correct temperature
            mdp_file = input_dir / "rest2.mdp"
            if mdp_file.exists():
    ...
    def print_temperature_summary(self) -> None:
        ...
        print("\n" + "="*70)
        print("REST2 Temperature Controller Summary")
        print("="*70)
        
        print(f"Total replicas: {self.n_replicas}")
        ...
        print("Temperature and Scaling Factors:")
        print(f"{'Replica':<8} {'Temp (K)':<10} {'λ factor':<12} {'√λ factor':<12}")
        print("-"*50)
        ...
        for replica in self.replicas:
            ...
            lambda_val = replica['scaling_factor']
            sqrt_lambda = np.sqrt(lambda_val)
            print(f"{idx:<8} {temp:<10.1f} {lambda_val:<12.6f} {sqrt_lambda:<12.6f}")
        
        print()
        print("Files generated for each replica:")
        print("  - topol.top: Replica-specific topology")
        print("  - rest2.mdp: Temperature-specific MDP file")
        print("  - plumed.dat: PLUMED file with PARTIAL_TEMPERING command")
        print("  - index.ndx: Index file for atom groups")
        
        if self.solute_atom_indices:
            print(f"\nPLUMED REST2 setup:")
            print(f"  - Solute atoms: {len(self.solute_atom_indices)} atoms")
            print(f"  - Reference temperature: {self.T_ref:.1f} K")
            print(f"  - PARTIAL_TEMPERING will scale solute interactions")
        else:
            print(f"\nWarning: No solute atom data provided.")
            print(f"  - PLUMED files contain placeholder atom lists")
            print(f"  - Manual editing of atom indices may be required")
